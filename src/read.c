	...
	if (cmdBuffer[minCmdOffset] == POINT) {
		...
		switch ((int)cmdBuffer[0]) {
			...
			case ELLIPSE:
				...
				(*cmdStruc_pt).m = char2int(cmdBuffer[13]);
				if ( !((unsigned int)(*cmdStruc_pt).x1 <= xMax && (unsigned int)(*cmdStruc_pt).y1 <= yMax && (unsigned int)(*cmdStruc_pt).dx != 0 && (unsigned int)(*cmdStruc_pt).dx <= xMax && (unsigned int)(*cmdStruc_pt).dy != 0 && (unsigned int)(*cmdStruc_pt).dy <= yMax && (unsigned int)(*cmdStruc_pt).m < 3) ) {
					return 0;
				}
				else {
					(*cmdStruc_pt).cmd = (int)cmdBuffer[0];
